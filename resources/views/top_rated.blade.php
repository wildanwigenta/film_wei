...
<div class="container">
  ...
  <div class="container-page">
    {{-- Kiri --}}
      {{-- Mundur 5 langkah --}}
        <a href="/top_rated/{{ $page <= 6 ? 1 : $page - 5 }}" class="page--link">
          <i class="fa-solid fa-angles-left" style="color: white"></i>
        </a>
      {{-- Mundur 5 langkah --}}

      {{-- Mundur 1 langkah --}}
        <a href="/top_rated/{{ $page == 1 ? 1 : $page - 1 }}" class="btn-page">
          <svg
    ...
    {{-- Tengah --}}
      <div class="link-container">
        @if ($page != 1)
          <a href="/top_rated/1" class="page--link {{ $page == 1 ? 'page--link--current' : ''}}">1</a>     
        @endif
        @if ($page >= 5)
          <span>...</span>
        @endif
        @for ($i = $page; $i < $page + 2; $i++)
          @if ($i - 2 >= 1 && $i - 2 != 1) 
            <a href="/top_rated/{{ $i-2 }}" class="page--link">{{ $i-2 }}</a> 
          @endif
        @endfor 
        <a href="/top_rated/{{ $page }}" class="page--link page--link--current">{{ $page }}</a>
        @for ($i = $page + 1; $i < $page + 3; $i++)
          @if ($i <= 500 && $i != 500)
            <a href="/top_rated/{{ $i }}" class="page--link">{{ $i }}</a> 
          @endif
        @endfor 
        @if ($page <= 496)
          <span>...</span>
        @endif
          @if ($page != 500)  
            <a href="/top_rated/500" class="page--link {{ $page == 500 ? 'page--link--current' : ''}}">500</a>
          @endif
      </div>
    {{-- Tengah --}}
    ...
    {{-- Kanan --}}
        {{-- Maju 5 langkah --}}
          <a href="/top_rated/{{ $page + 1 }}" class="btn-page">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
        ...
        {{-- Maju 1 langkah --}}
          <a href="/top_rated/{{ $page >= 494 ? 500 : $page + 5 }}" class="page--link">
